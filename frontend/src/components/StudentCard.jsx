                    ...
                    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ padding: '0 14px', marginTop: '4px', zIndex: 10 }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '7px', fontFamily: '"Arial", sans-serif', color: '#0f172a', fontWeight: 600 }}>
                                <tbody>
                                    <tr><td style={{ width: '32%', paddingBottom: '2px' }}>Nama</td><td style={{ width: '4%' }}>:</td><td style={{ textTransform: 'uppercase' }}>{profile?.full_name}</td></tr>
                                    <tr><td style={{ paddingBottom: '2px' }}>NIK / NIS</td><td>:</td><td>{profile?.nik} / {profile?.nisn}</td></tr>
                                    <tr><td style={{ paddingBottom: '2px' }}>Tempat, Tgl Lahir</td><td>:</td><td>{profile?.birth_place}, {profile?.birth_date}</td></tr>
                                    <tr><td style={{ paddingBottom: '2px' }}>Agama</td><td>:</td><td>{profile?.religion}</td></tr>
                                    <tr><td style={{ paddingBottom: '2px' }}>Konsentrasi Keahlian</td><td>:</td><td>{profile?.major?.name || ''}</td></tr>
                                    <tr><td style={{ verticalAlign: 'top' }}>Alamat</td><td style={{ verticalAlign: 'top' }}>:</td><td style={{ lineHeight: '1.2' }}>{profile?.address}</td></tr>
                                </tbody>
                            </table>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 14px', marginTop: 'auto', marginBottom: '8px', alignItems: 'flex-end', zIndex: 10 }}>
                            <div style={{ background: '#e11d48', color: 'white', padding: '4px 6px', fontSize: '5.5px', fontWeight: 800, textAlign: 'center', lineHeight: '1.2' }}>BERLAKU SELAMA<br />MENJADI SISWA</div>
                            <div style={{ textAlign: 'center', fontSize: '5px', color: '#0f172a', fontWeight: 700, width: '75px', position: 'relative' }}>
                                ...
                                <div style={{ position: 'relative', height: '35px', margin: '2px 0' }}>
                                    {template?.signature_image_path && <img src={`/api/image?path=${template.signature_image_path}`} crossOrigin="anonymous" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '120%', maxHeight: '42px', objectFit: 'contain', mixBlendMode: 'multiply' }} />}
                                </div>
                                <div style={{ fontWeight: 800, textDecoration: 'underline' }}>{template?.principal_name || 'NAMA KEPALA SEKOLAH'}</div>
                                <div>{template?.principal_nip || 'NIP/NBM. -'}</div>
                            </div>
                        </div>
                    </div>
                ) : (
